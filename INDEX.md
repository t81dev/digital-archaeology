...
- [SteamOS: Gaming-Session Operating System & Vendor Linux Substrate](excavations/steamos.md) — *Dual-session launcher-as-shell model, Gamescope micro-compositor presentation path, Proton Win32/Direct3D to POSIX/Vulkan translation stack, controller-first ergonomics, and immutable A/B system root updates.*
...
- [Wayland: Compositor-Centered Display Protocol & Surface Architecture](excavations/wayland.md) — *Asynchronous wire protocol, double-buffered surface transactions (`wl_surface`), DMA-BUF zero-copy presentation, compositor-centric display/input authority, seat isolation, and capability portals (`xdg-desktop-portal`).*
